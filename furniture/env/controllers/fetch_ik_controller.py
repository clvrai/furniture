...
class FetchIKController(Controller):
    """
    Inverse kinematics for the Fetch robot, using Pybullet and the urdf description
    files. Loads a Fetch robot into an internal Pybullet simulation, and uses it to
    ...
    def setup_inverse_kinematics(self):
        ...
        self.robot_urdf = pjoin(
            self.bullet_data_path, "fetch_description/urdf/fetch_arm.urdf"
        )
    ...
    def joint_positions_for_eef_command(self, dpos, rotation):
        ...
        # from the final joint by the 'quat' value for 'right_hand' specified
        # in fetch/robot.xml
