...
class GAILAgent(BaseAgent):
    def __init__(self, config, ob_space, ac_space, actor, critic):
        ...
        self._discriminator = Discriminator(
            self._config, self._ob_space, self._ac_space
        )
        ...
        self._discriminator_optim = optim.Adam(
            self._discriminator.parameters(), lr=config.lr_bc
        )
    ...
    def _compute_gae(self, rollouts):
        ...
        for t in reversed(range(T)):
            ...
            delta = (
                rew[t]
                + self._config.discount_factor * vpred[t + 1] * nonterminal
                - vpred[t]
            )
            adv[t] = lastgaelam = (
                delta
    ...
    def _update_policy(self, transitions):
        ...
        surr2 = (
            torch.clamp(
                ratio, 1.0 - self._config.clip_param, 1.0 + self._config.clip_param
            )
            * adv
        )
