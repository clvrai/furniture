...
class Trainer(object):
    def __init__(self, config):
        ...
        if config.ob_norm:
            ...
            self._g_norm = Normalizer(goal_space, default_clip_range=config.clip_range, clip_obs=config.clip_obs)
        else:
        ...
        if config.goal_type == 'tcn':
            ...
            self._goal_estimator = BasicTrackingEstimator.from_checkpoint(config.goal_estimator_path)
        ...
        self._agent = get_agent_by_name(config.algo)(
            config, self._env, ob_space, goal_space, ac_space, self._ob_norm, self._g_norm
        )

        self._meta_agent = MetaAgent(config, self._env, ob_space, goal_space,
                                     self._ob_norm, self._g_norm)

        # build rollout runner and replay buffer
        self._runner = RolloutRunner(config, self._env, self._meta_agent, self._agent,
                                     self._goal_estimator, self._ob_norm, self._g_norm)
    ...
    def _save_ckpt(self, ckpt_num, update_iter):
        ...
        if self._config.ob_norm:
            state_dict.update({'ob_norm_state_dict': self._ob_norm.state_dict(),
    ...
    def _save_rollouts(self, step, meta_rollout, ep_rollout, is_train=True):
        ...
        rollout_path = os.path.join(self._config.rollout_dir, tag + ('_rollouts_%08d.pkl' % step))
        with gzip.open(rollout_path, 'wb') as f:
            rollouts = {
                'meta_rollout': meta_rollout,
                'ep_rollout': ep_rollout
            }
            pickle.dump(rollouts, f)

    def _log_train(self, step, meta_rollout, ep_rollout, meta_train_info, train_info, ep_info):
    ...
    def _plot_rollout(self, step, meta_rollout, rollout, is_train=True):
        ...
        ax.scatter(goals[:,0][meta_idxs], goals[:,1][meta_idxs], s=100, marker='o',
        ...
        for i, a in enumerate(trajectories[:-1]):
            b = trajectories[i+1]
            an = ax.annotate("", xy=b[:2], xytext=a[:2],
                             arrowprops=dict(arrowstyle='simple', color=line.get_color()), zorder=1)

        if self._env.name == 'robot_push':
            bound_rect = patches.Rectangle((0.671 - 0.01, -0.075 - 0.01), 0.046 + 0.02, 0.055 + 0.02,0,linewidth=1,edgecolor='r',facecolor='r',alpha=0.2, hatch='/')
            rect = patches.Rectangle((0.671, -0.075), 0.046, 0.055,0,linewidth=1,edgecolor='r',facecolor='r',alpha=0.8)
        elif self._env.name =='push':
            bound_rect = patches.Rectangle((0.61 - 0.02, 0 - 0.02), 0.04, 0.04,0,linewidth=1,edgecolor='r',facecolor='r',alpha=0.2, hatch='/')
            rect = patches.Rectangle((0.61 - 0.02, 0.0 - 0.02), 0.04, 0.04,0,linewidth=1,edgecolor='r',facecolor='r',alpha=0.8)
        ...
        while step < config.max_global_step:
            ...
            if self._is_chef:
                ...
                if update_iter % config.log_interval == 0:
                    ...
                    st_time = time()
                    st_step = step
                    self._log_train(step, meta_rollout, rollout, meta_train_info, train_info, ep_info)
                    ep_info = defaultdict(list)

                if update_iter % config.evaluate_interval == 0:
                    logger.info('Evaluate at %d', update_iter)
                    rollout, meta_rollout, info = self._evaluate(step=step, record=config.record)
    ...
    def _evaluate(self, step=None, record=False, idx=None):
        rollout, meta_rollout, info = self._runner.run_episode(is_train=False,
                                                               record=record,
                                                               step=step, idx=idx)
    ...
    def evaluate(self):
        ...
        for i in trange(self._config.num_eval):
            ...
            rollout, meta_rollout, info = self._evaluate(step=step, record=self._config.record, idx=i)
        ...
        keys = ['hrl_success', 'env_success', 'covered_frames', 'demo_len', 'coverage',
                'fail_low', 'fail_meta']
        os.makedirs('result', exist_ok=True)
        with h5py.File('result/{}.hdf5'.format(self._config.run_name), 'w') as hf:
            for k in ['hrl_success', 'env_success', 'covered_frames', 'demo_len',
                      'fail_low', 'fail_meta']:
